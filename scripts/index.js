...
/** ---------------- Вспомогательная функция для удаления HTML ---------------- */
/** Удаляет все HTML-теги из строки. */
function stripHtmlTags(str) {
    if (!str) return str;
    // Используем регулярное выражение для удаления любых тегов <...>
...
/** ---------------- Сохранить результат (С ИЗМЕНЕНИЯМИ) ---------------- */
/**
 * Очищает результат броска от HTML-тегов, прежде чем сохранить его.
 */
async function saveRolledValues(tokenDocument, result) {
    // 1. Очищаем имя от HTML-тегов
    const cleanedName = result?.name ? stripHtmlTags(String(result.name)) : undefined;
    
    // 2. Очищаем био от HTML-тегов, если нужно
    const cleanedBio = result?.bio ? stripHtmlTags(String(result.bio)) : undefined;
    ...
    if (cleanedBio && result?.bioDataPath && tokenDocument.actor) {
        // Сохраняем очищенное био
        await tokenDocument.actor.update({ [result.bioDataPath]: cleanedBio });
    }
}
